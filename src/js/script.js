var mode = "exploration"; var codetraceColor = 'white'; actionsWidth = 0; function highlightLine(lineNumbers) {
    $('#codetrace p').css('background-color', colourTheThird).css('color', codetraceColor); if (lineNumbers instanceof Array) {
...
function showActionsPanel() {
    if (!isActionsOpen()) {
        if ($('#actions-hide span').length) { $('#actions-hide span').removeClass('rotateLeft').addClass('rotateRight'); } else { $('#actions-hide img').removeClass('rotateLeft').addClass('rotateRight'); }
        $('#actions').animate({ width: "+=" + actionsWidth, }); if (isMobile()) { hideStatusPanel(); hideCodetracePanel(); }
    }
}
function hideActionsPanel() {
    if (isActionsOpen()) {
        if ($('#actions-hide span').length) { $('#actions-hide span').removeClass('rotateRight').addClass('rotateLeft'); } else { $('#actions-hide img').removeClass('rotateRight').addClass('rotateLeft'); }
...
function triggerRightPanels() {
    hideEntireActionsPanel(); if (!isMobile()) { showCodetracePanel(); }
    showStatusPanel();
}
function extractQnGraph(graph) {
    var vList = graph.internalAdjList; var eList = graph.internalEdgeList; for (var key in vList) { var temp; var v = vList[key]; temp = v.cxPercentage; v.cxPercentage = v.cx; v.cx = (temp / 100) * MAIN_SVG_WIDTH; temp = v.cyPercentage; v.cyPercentage = v.cy; v.cy = (temp / 100) * MAIN_SVG_HEIGHT; }
    return graph;
}
...
function openSlide(slide, callback) {
    mode = 'e-Lecture'; isPlaying = false; if (typeof gw != 'undefined' && gw != null && typeof gw.stop == 'function' && isPlaying) {
        try { gw.stop(); }
        catch (err) { }
    }
...
function initUI() {
    var actionsHeight = ($('#actions p').length) * 27 + 10; $('#actions').css('height', actionsHeight); $('#actions').css('width', actionsWidth); var actionsIconHeight = 0; if ($('#actions-hide span').length) { actionsIconHeight = $('#actions-hide span').height(); } else { actionsIconHeight = $('#actions-hide img').height(); }
...
let speedVal = 1; function createPlaybackSpeedSlider(defaultPlaybackSpeed) {
    $("#speed-input").slider({
        range: false, min: 1000, max: 2000, value: defaultPlaybackSpeed * 1000 + 500, step: 250, change: function (event, ui) { gw.setAnimationDuration(2200 - ui.value); }, slide: function (event, ui) {
...
$(function () {
    $("#progress-bar").slider({ range: "min", min: 0, value: 0, slide: function (event, ui) { gw.pause(); gw.jumpToIteration(ui.value, 0); }, stop: function (event, ui) { if (!isPaused) { setTimeout(function () { gw.play(); }, 500); } } }); initUI(); $('#mode-button').click(function () { $('#other-modes').toggle(); }); $('#mode-menu').hover(function () { $('#other-modes').show(); }, function () { $('#other-modes').hide(); }); $('#mode-menu a').hover(function () { $(this).css("background", surpriseColour); }, function () { $(this).css("background", "rgb(0,0,0)"); }); $('#other-modes a').click(function () {
        ...
        if (isCodetraceOpen())
            ...
            showCodetracePanel();
    }); $('#actions-hide').click(function () { if (isActionsOpen()) { hideEntireActionsPanel(); } else { showActionsPanel(); } }); $('.electure-end').click(end_eLecture); $('.electure-prev').click(function () { openSlide($(this).attr('data-nextid')); }); $('.electure-next').click(function () { openSlide($(this).attr('data-nextid')); }); $(document).keydown(function (event) {
        if (event.which == 32) {
            if (mode != "e-Lecture") {
                if (isPaused)
                    play(); else
            ...
            if ($("#dark-overlay").css('display') == 'none') {
                stop(); if (mode == "e-Lecture") { $(".menu-highlighted").removeClass("menu-highlighted"); end_eLecture(); }
        ...
        else if (event.which == 187) { var d = (2200 - gw.getAnimationDuration()) + 100; $("#speed-input").slider("value", d <= 2000 ? d : 2000); }
    });
}); var isPaused = false; function isAtEnd() { return (gw.getCurrentIteration() == (gw.getTotalIteration() - 1)); }
...
function play() {
    if (isPlaying) {
        isPaused = false; $('#pause').show(); $('#play').hide(); $('#mobile-playback-overlay').removeClass('paused'); $('#mobile-playback-overlay').addClass('playing'); $('#mobile-playback-pause').show(); $('#mobile-playback-play').hide(); if (isAtEnd())
            gw.replay(); else
            gw.play();
    }
}
function stepForward(numFrames) {
    if (isPlaying) {
        if (numFrames && Number.isInteger(numFrames)) { gw.jumpToIteration(gw.getCurrentIteration() + 7, 250); } else
            gw.forceNext(250);
    }
}
